...
public class GBPR {
    ...
    // group size
    public static int setsizeG = 3;
    // the tradeoff parameter in group preference
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int k=0; k < args.length; k++) {
            ...
            else if (args[k].equals("setsizeG")) setsizeG = Integer.parseInt(args[++k]);
        ...
        System.out.println("setsizeG: " + Float.toString(setsizeG));
    ...
    public static void train() throws FileNotFoundException {
        for(int iter = 0; iter < num_iterations; iter++) {
            for(int iter_rand = 0; iter_rand < n; iter_rand++) {
                ...
                // sample an item $i$ from $I_u^+$
                HashSet<Integer> itemSet = TrainData.get(u);
                List<Integer> itemList = new ArrayList<Integer>(itemSet);
                int random_i = (int) Math.floor(Math.random() * itemList.size());
                int i = itemList.get(random_i);
                
                // sample an item $j$ from $I_u^*$, 
                int j = 0;
                do {
                    j = (int) Math.floor(Math.random() * m) + 1;
                } while( !ItemSetWhole.contains(j) || itemSet.contains(j) );
                
                // sample setsizeG users rondomly
                HashSet<Integer> userSet = TrainDataItem2User.get(i);
	    		int userSetSize = userSet.size();
	    		List<Integer> userList = new ArrayList<Integer>(userSet);
	    		
	    		HashSet<Integer> userSetG = new HashSet<Integer>();
	    		userSetG.add(u);

	    		int k = 1;   // because there is one user already
	    		while( k < setsizeG && k < userSetSize) {
	    	    	int random_w = (int) Math.floor(Math.random() * userSetSize);
	    	    	int w = userList.get(random_w);
	    	    	
	    			if ( !userSetG.contains(w) ) {
	    				userSetG.add(w);
	    				k += 1;
	    			}
	    			else {
	    		...
	    		int userSetSizeG = userSetG.size();
	    	    ...
	    	    float r_Guiuj = lambda*r_Gi + (1-lambda) * r_ui - r_uj;
                


                // Step 3: updating

                // calculate $loss_Guiuj$
                float loss_Guiuj = - 1f / (1f + (float) Math.pow(Math.E, r_Guiuj) );
                
                // update $U_{w\cdot}$
	    		for (int w : userSetG) {
	    			if(w == u) {
	    				for (int f = 0; f < d; f++) {
	    					U[u][f] = U[u][f] - eta * ( loss_Guiuj * ( V[i][f] * ( 1-lambda + lambda/userSetSizeG ) - V[j][f] ) + alpha_u * U[u][f] );
	    		    	}	    			
	    			}
	    			else {
	        			for (int f = 0; f < d; f++) {
	        				U[w][f] = U[w][f] - eta * ( loss_Guiuj * V[i][f] * lambda/userSetSizeG + alpha_u * U[w][f] / userSetSizeG );	    			
	    		    	}
	    			}
	    		}
	    		...
	    		// update $V_{i\cdot}$
	        	for (int f = 0; f < d; f++) {	
	    			V[i][f] = V[i][f] - eta * ( loss_Guiuj * ( (1-lambda) * U[u][f] + lambda * U_G[f] ) + alpha_v * V[i][f] );
	    	   	}

	    		// update $V_{j\cdot}$
	    		for (int f = 0; f < d; f++) {
	    			V[j][f] = V[j][f] - eta * ( loss_Guiuj * (-U[u][f]) + alpha_v * V[j][f] );
	    	    }
               
	    		// --- update $b_i$	    		
	    		biasV[i] = biasV[i] - eta * ( loss_Guiuj + beta_v * biasV[i] );
                
	    		// --- update $b_j$
	    		biasV[j] = biasV[j] - eta * ( loss_Guiuj * (-1) + beta_v * biasV[j] );
            }
        }
    }
