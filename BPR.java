    ...
    public static void train() throws FileNotFoundException {
        for(int iter = 0; iter < num_iterations; iter++) {
            for(int iter_rand = 0; iter_rand < n; iter_rand++) {
                ...
                // calculate $loss_uiuj$
                float loss_uiuj = - 1f / (1f + (float) Math.pow(Math.E, r_uiuj) );

  
                // update $U_{u\cdot}$
                for (int f=0; f<d; f++) {
                    U[u][f] = U[u][f] - eta * ( loss_uiuj * ( V[i][f]  - V[j][f] ) + alpha_u * U[u][f] );
                }	    

                
	    		// update $V_{i\cdot}$
	        	for (int f=0; f<d; f++) {	
	    			V[i][f] = V[i][f] - eta * ( loss_uiuj * U[u][f] + alpha_v * V[i][f] );
	    	   	}


	    		// update $V_{j\cdot}$
	    		for (int f=0; f<d; f++) {
	    			V[j][f] = V[j][f] - eta * ( loss_uiuj * (-U[u][f]) + alpha_v * V[j][f] );
	    	    }


	    		// --- update $b_i$	    		
	    		biasV[i] = biasV[i] - eta * ( loss_uiuj + beta_v * biasV[i] );


	    		// --- update $b_j$
	    		biasV[j] = biasV[j] - eta * ( loss_uiuj * (-1) + beta_v * biasV[j] );
            }
        }
    }
